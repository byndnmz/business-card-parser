...
- **OCR/Parser:** Sağlayıcı adaptör mimarisi (`OCR_PROVIDER`). **Varsayılan: RapidOCR**
  — tamamen **YEREL/OFFLINE** (Python sidecar, `ocr-service/`; görsel sunucudan çıkmaz).
  Akış: OpenCV ön-işleme (perspektif düzeltme + deskew + koşullu CLAHE/denoise/upscale) →
  RapidOCR (det+cls+rec, **Latin/Türkçe** model) → **layout** (kutu→satır→blok, punto
  sinyali) → **çıkarıcılar** (`extractors/`: email/telefon/url/adres/vkn/sosyal +
  çok-sinyalli isim/unvan/şirket skorlama) → **çapraz doğrulama** + güven skoru →
  `ParsedCard`. QR/vCard varsa alanlar doğrudan oradan alınır. `OCR_PROVIDER=gemini`
  yalnızca regresyon karşılaştırması için kalır; `tesseract` alternatif offline yoldur.
...
**Önkoşul:** Node.js 18+ ve (yerel OCR için) Python 3.10+
...
# 2) Yerel OCR sidecar (RapidOCR) — ayrı terminal
cd ocr-service
python -m venv .venv
.venv/Scripts/pip install -r requirements.txt   # Linux/mac: .venv/bin/pip
.venv/Scripts/python app.py                      # http://127.0.0.1:8765
#   İlk açılışta Latin/Türkçe ONNX modeli indirilir. Doğrulama: python selftest.py

# 3) Uygulama
npm run dev                  # http://localhost:3000
```

> OCR sidecar çalışmıyorsa OCR akışı açık uyarıyla güvenli geri-dönüşe geçer
> (çökme yok). `ocr-service/README.md` Türkçe model ayarını ve API'yi anlatır.
...
- `OCR_PROVIDER` (varsayılan `rapidocr`) / `RAPIDOCR_SERVICE_URL` — yerel OCR.
  `GEMINI_*` yalnızca `OCR_PROVIDER=gemini` (regresyon) içindir.
