        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-[#080c14] p-4 rounded-lg border border-[#1e293b] space-y-3">
            ...
            <div className="text-xs font-bold font-mono text-slate-200">
              {systemHealth.geminiCognitiveEngine || "OFFLINE_FALLBACK_SIMULATION"}
            </div>
            <div className="text-[10px] text-[#94a3b8] font-mono space-y-1">
              <div>• Sağlayıcı (Adapter): {systemHealth.ocrProvider || "gemini"}</div>
              <div>• SDK Model: {systemHealth.ocrModel || "gemini-2.5-flash"}</div>
              <div>• Doğrulama: Regex + structured output</div>
            </div>
          </div>
        </div>
      )}
