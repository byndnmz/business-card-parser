...
> **OCR artık tamamen YEREL** (RapidOCR sidecar, `ocr-service/`) — GEMINI_API_KEY
> GEREKMEZ. Sidecar ayrı bir servis olarak dağıtılır (§3C) ve `RAPIDOCR_SERVICE_URL`
> ile bağlanır. Gemini yalnızca `OCR_PROVIDER=gemini` (regresyon) için isteğe bağlıdır.
...
## 3C. OCR Sidecar (RapidOCR) dağıtımı — YEREL/OFFLINE OCR

OCR artık `ocr-service/` altındaki Python servisinde çalışır (görsel sunucudan çıkmaz).
İki dağıtım deseni:

**Desen 1 — Ayrı Cloud Run servisi (önerilen):**
```bash
gcloud run deploy bcip-ocr --source ./ocr-service --region europe-west1 \
  --no-allow-unauthenticated --memory 2Gi --cpu 2
#   Ardından ana uygulamaya bağla:
#   RAPIDOCR_SERVICE_URL=https://bcip-ocr-xxxx.run.app  (yalnızca iç erişim)
```
- ONNX modelleri imaja gömülür (çalışma zamanında indirme yok = gerçek offline).
  Türkçe için `RAPIDOCR_REC_LANG=latin` zaten varsayılandır.
- Ana servis ile sidecar arasını **iç ağ/IAM** ile sınırla (sidecar'ı public yapma).

**Desen 2 — Tek konteyner (yan yana süreç):** Ana Node imajına Python + `ocr-service/`
eklenip sidecar `127.0.0.1:8765`'te başlatılır (`RAPIDOCR_SERVICE_URL` varsayılanı).
Basit ama imaj büyür.

> Sidecar erişilemezse OCR akışı **açık uyarıyla** güvenli geri-dönüşe geçer (çökme yok).
...
- Bir kartvizit yükle → gerçek OCR çıktısı (`provider: rapidocr`) gelmeli.
